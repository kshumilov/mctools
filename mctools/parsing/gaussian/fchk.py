...
from typing import Type, TYPE_CHECKING, Optional, IO, AnyStr, Any, Literal
...
class FchkStepper(FileStepper):
    @staticmethod
    def process_data_type(char: str, /) -> Type[ScalarType]:
        match char:
            case 'R':
                return np.float_
            case 'I':
    ...
    ScalarType = np.float_ | np.int_ | np.str_

    MethodPatt = ProcessedPattern(
        r'(?P<calc>[a-zA-Z]+)\s*'
    ...
    def read_scalar(self, header: str) -> int | float | str:
        line, *_ = self.step_skip_until(header, check_last_read=True)
    ...
    def read_array(self, header: str) -> ndarray[ScalarType, dtype[ScalarType]]:
        ...
        arr_info = patt.match(line)

        arr = []
        while self.step_forward() and len(arr) < arr_info.size:
            arr.extend(self.line.split())
